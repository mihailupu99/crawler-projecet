...
class Article(Base):
    ...
    # Generation checkmarks / timestamps
    has_image_generated: Mapped[bool] = mapped_column(Boolean, default=False)
    image_generated_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    has_text_generated: Mapped[bool] = mapped_column(Boolean, default=False)
    text_generated_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)

    assets: Mapped[list[Asset]] = relationship(
        back_populates="article", cascade="all, delete-orphan"
    )
    texts: Mapped[list[TextRow]] = relationship(
        back_populates="article", cascade="all, delete-orphan"
    )
...
class Asset(Base):
    ...
    # Optional: store the bytes in DB too
...
class Run(Base):
    ...
    input_text_id: Mapped[int | None] = mapped_column(
        ForeignKey("texts.id"), nullable=True
    )
    status: Mapped[RunStatus] = mapped_column(
        Enum(RunStatus), default=RunStatus.queued
    )
